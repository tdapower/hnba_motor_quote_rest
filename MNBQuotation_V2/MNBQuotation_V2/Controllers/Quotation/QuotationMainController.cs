...
namespace MNBQuotation_V2.Controllers.Quotation
{
    ...
    public class QuotationMainController : ApiController
    {
        string ConnectionString = ConfigurationManager.ConnectionStrings["OracleConString"].ToString();

        public IEnumerable<QuotationMain> Get()
        {
            List<QuotationMain> quotationMainList = new List<QuotationMain>();
            ...
            if (user.Company != Properties.Settings.Default.MNBQGeneralCompanyName)
            {
                ...
                sql = " SELECT " +
                               ...
                               "CASE WHEN MM.YEAR_OF_MANU         IS NULL THEN ''  ELSE MM.YEAR_OF_MANU        END, " +
            ...
            try
            {
                ...
                quotationMainList = (from DataRow drow in dt.Rows
                                     select new QuotationMain()
                                     {
                                         ...
                                         YearOfManu = drow[23].ToString(),
                                         YearOfManufactureValidationId = drow[24].ToString(),
                                         MakeModelCode = drow[25].ToString()
        ...
        public QuotationMain Get(int id)
        {
            ...
            try
            {
                ...
                if (dr.HasRows)
                {
                    ...
                    quotationMain.YearOfManu = dr[23].ToString();
                    quotationMain.YearOfManufactureValidationId = dr[24].ToString();
                    quotationMain.MakeModelCode = dr[25].ToString();
        ...
        //[ValidateModel]
        //[HttpPost]
        //public HttpResponseMessage AddQuotationMainDetails([FromBody] QuotationMain quotationMain)
        //{     
        ...
        [ValidateModel]
        [HttpPost]
        public string AddQuotationMainDetails(QuotationMain quotationMain)
        {
            ...
            if (CheckVehicleNoAlreadyAvailable(quotationMain.JobId, quotationMain.VehicleChasisNo))
            {
                ...
                return "Vehicle/Chassi Number Already available and cannot duplicated";
            ...
            int MNBQMotorThirdPartyProductCode = Properties.Settings.Default.MNBQMotorThirdPartyProductCode;
            ...
            try
            {
                ...
                cmd = new OracleCommand("INSERT_MNBQ_MAIN");
                ...
                if (quotationMain.ProductCode == MNBQMotorThirdPartyProductCode)
                {
                    cmd.Parameters.Add("V_SUM_INSURED", OracleType.Double).Value = 0.00;
                }
                else
                {
                ...
                cmd.Parameters.Add("V_YEAR_OF_MANU", OracleType.VarChar).Value = quotationMain.YearOfManu;
                ...
                //cmd.Parameters.Add("V_AR_EXCITATION", OracleType.VarChar).Value = "0";
                //cmd.Parameters.Add("V_AR_REBATE", OracleType.VarChar).Value ="0";
                ...
                if (user.Company != Properties.Settings.Default.MNBQGeneralCompanyName)
                {
                    QuotationCoverController quotationCoverController = new QuotationCoverController();
                    quotationCoverController.saveDefaultCovers(Convert.ToInt32(newJobID), user);
                }
        ...
        [HttpPost]
        public string UpdateQuotationMainDetails(QuotationMain quotationMain)
        {
            ...
            try
            {
                ...
                cmd = new OracleCommand("MODIFY_MNBQ_MAIN");
                ...
                if (user.Company != Properties.Settings.Default.MNBQGeneralCompanyName)
                {
                    ...
                    quotationCoverController.DeleteCovers(Convert.ToInt32(quotationMain.JobId), 0);
                    quotationCoverController.saveDefaultCovers(Convert.ToInt32(quotationMain.JobId), user);
                }
        ...
        public void CalculatePremium(int jobId, int revisionId, int productCode)
        {
            try
            {
                ...
                int MNBQMotorGuardProductCode = Properties.Settings.Default.MNBQMotorGuardProductCode;
                int MNBQMotorGuardExtraProductCode = Properties.Settings.Default.MNBQMotorGuardExtraProductCode;
                int MNBQMotorThirdPartyProductCode = Properties.Settings.Default.MNBQMotorThirdPartyProductCode;

                OracleCommand cmd = null;


                if (productCode == MNBQMotorGuardProductCode)
                {
                    cmd = new OracleCommand("MNBQ_CALC_MG");
                }
                else if (productCode == MNBQMotorGuardExtraProductCode)
                {
                    cmd = new OracleCommand("MNBQ_CALC_MGEX");
                }
                else if (productCode == MNBQMotorThirdPartyProductCode)
                {
                    cmd = new OracleCommand("MNBQ_CALC_THIRDPARTY");
                }
        ...
        public string CalculateAndGetPremium(QuotationCalculate quotationCalculate)
        {
            ...
            try
            {
                ...
                if (quotationCalculate.ProductCode == MNBQMotorGuardProductCode)
                {
                    cmd = new OracleCommand("MNBQ_CALC_MG");
                }
                else if (quotationCalculate.ProductCode == MNBQMotorGuardExtraProductCode)
                {
                    cmd = new OracleCommand("MNBQ_CALC_MGEX");
                }
                else if (quotationCalculate.ProductCode == MNBQMotorThirdPartyProductCode)
                {
                    cmd = new OracleCommand("MNBQ_CALC_THIRDPARTY");
                }
        ...
        private bool CheckVehicleNoAlreadyAvailable(int jobId, string vehicleNo)
        {
            ...
            try
            {
                ...
                if (jobId > 0)
                {
                    sql = "SELECT VEHICLE_CHASIS_NO  FROM mnbq_main T " +
        ...
        //[HttpGet]
        //public QuotationResult GetQuotationResultByJobId(int id)
        //{
        //    var uIdentity = Thread.CurrentPrincipal.Identity;
        //    UserAccountController uc = new UserAccountController();
        //    UserAccount user = uc.getUserFromUserName(uIdentity.Name.ToString());
        ...
        //        quotationResult = GetQuotationResultForExtByJobId(id);
        //    }
        //    else
        //    {
        ...
        public QuotationResult GetQuotationResultForInternalByJobId(int id)
        {
            ...
            string sql = "select " +
                             ...
                             " from mnb_quot_get_results t " +
                             " inner join mnbq_main mm on mm.JOB_ID=t.JOB_ID " +
        ...
        //public QuotationResult GetQuotationResultForExtByJobId(int id)
        //{
        //    QuotationResult quotationResult = new QuotationResult();
        //    OracleConnection con = new OracleConnection(ConnectionString);
        //    OracleDataReader dr = null;
        ...
        private void UpdateQuotationNumber(int jobId, int revisionNo, string branchId, int productCode, double sumInsured, string userId)
        {
            ...
            try
            {
                ...
                cmd = new OracleCommand("UPDATE_MNBQ_QUOT_NUMBER");
        ...
        //public HttpResponseMessage GetCalculation(QuotationViewRequest quotationViewRequest)
        //{
        //    HttpResponseMessage response = new HttpResponseMessage(HttpStatusCode.OK);

        //    if (quotationViewRequest.JobId== 0)
        //    {
        ...
        //        if (quotationViewRequest.ProductCode == MNBQMotorGuardProductCode)
        //        {
        //            crystalReport.Load(System.Web.Hosting.HostingEnvironment.MapPath("~/Reports/MotorQuotationCalculation.rpt"));
        //        }
        //        else if (quotationViewRequest.ProductCode == MNBQMotorGuardExtraProductCode)
        //        {
        //            crystalReport.Load(System.Web.Hosting.HostingEnvironment.MapPath("~/Reports/MotorGuardExtraCalculation.rpt"));
        //        }
        //        else if (quotationViewRequest.ProductCode == MNBQMotorThirdPartyProductCode)
        //        {
        //            crystalReport.Load(System.Web.Hosting.HostingEnvironment.MapPath("~/Reports/ThirdPartyQuotationCalculation.rpt"));
        //        }
        ...
        //        crystalReport.SetParameterValue("P_JOB_ID", quotationViewRequest.JobId);
        //        crystalReport.SetParameterValue("P_REVISION_NO", quotationViewRequest.RevisionId);

        //        var document = crystalReport.ExportToStream(ExportFormatType.PortableDocFormat);

        //        response.Content = new StreamContent(document);
        //        response.Content.Headers.ContentDisposition = new System.Net.Http.Headers.ContentDispositionHeaderValue("attachment");
        //        response.Content.Headers.ContentDisposition.FileName = "Calculation.pdf";
        //    }
        //    catch (Exception ex)
        //    {
        //        throw;
        ...
        //public HttpResponseMessage GetQuotation(QuotationViewRequest quotationViewRequest)
        //{
        //    HttpResponseMessage response = new HttpResponseMessage(HttpStatusCode.OK);
        ...
        //        if (quotationViewRequest.ProductCode == MNBQMotorGuardProductCode)
        //        {
        //            crystalReport.Load(System.Web.Hosting.HostingEnvironment.MapPath("~/Reports/MotorQuotation.rpt"));
        //        }
        //        else if (quotationViewRequest.ProductCode == MNBQMotorGuardExtraProductCode)
        //        {
        //            crystalReport.Load(System.Web.Hosting.HostingEnvironment.MapPath("~/Reports/MotorGuardExtraCalculation.rpt"));
        ...
        //        else if (quotationViewRequest.ProductCode == MNBQMotorThirdPartyProductCode)
        //        {
        //            crystalReport.Load(System.Web.Hosting.HostingEnvironment.MapPath("~/Reports/ThirdPartyQuotation.rpt"));
        //        }
        ...
        //        response.Content = new StreamContent(document);
        //        response.Content.Headers.ContentDisposition = new System.Net.Http.Headers.ContentDispositionHeaderValue("attachment");
        //        response.Content.Headers.ContentDisposition.FileName = "Quotation.pdf";
        //    }
        //    catch (Exception ex)
        //    {
        //        throw;
        ...
        public HttpResponseMessage GetCalculation(string jobId, int productCode)
        {
            ...
            try
            {
                ...
                //crystalReport.SetDatabaseLogon("hnba_crc", "HNBACRC", "RACPROD", "");
                crystalReport.SetDatabaseLogon("hnba_crc", "HNBACRC", "TESTCRC", "");
